window.onload = function () {
    ...
    function terminarPartida() {
        ...
        if (marcadorJugador1 === 10 || marcadorJugador2 === 10) {
            ...
            guardarResultado(); // Guardamos el resultado de la partida.
    ...
    // Function para guardar el resultado.

    function guardarResultado() {
        const resultadoActual = `${marcadorJugador1},${marcadorJugador2}`;
    
        let historial = localStorage.getItem("historialPong") || ""; // Si no existe historial ponemos que no hay nada en el historial
    
        // Si hay historial, ponemos el nuevo resultado, sino ponemos el resultado actual
        historial = historial ? `${historial}|${resultadoActual}` : resultadoActual;
    
        localStorage.setItem("historialPong", historial); // Guardamos el historial actualizado en localStorage
    
        mostrarHistorial(); 
    }

    // Mostrar historial de los resultados de las partidas.

    function mostrarHistorial() {
        const historialDiv = document.getElementById("historial-content");
    
        let historial = localStorage.getItem("historialPong") || ""; // Recuperamos el historial del localStorage
    
        if (!historial) {
            historialDiv.innerHTML = "<p>No hay historial de partidas.</p>"; // Si no hay historial, mostramos este mensaje.
            return;
        }
    
        let partidas = historial.split("|"); // Dividimos el historial por "|"
    
        let contenidoHTML = ""; // Aqui ponemos el contenido a mostrar
    
        for (let i = 0; i < partidas.length; i++) {
            contenidoHTML += `<p>Partida ${i + 1}: ${partidas[i]}</p>`; // Hacemos que se muestre en el HTML para cada partida
        }
    
        historialDiv.innerHTML = contenidoHTML; // Mostramos el contenido en el HTML
    }
    ...
    mostrarHistorial(); // Mostramos el historial
